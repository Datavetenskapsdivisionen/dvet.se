...
const readInvoiceData = () => {
    try {
        const file = fs.readFileSync("backend/invoice-data.json");
...
const writeInvoiceData = (res, data) => {
    fs.writeFile("backend/invoice-data.json", JSON.stringify(data), error => {
...
const getInvoice = async (req, res) => {
    ...
    const dir = "/backend/latex/invoice/pdf";
...
const createInvoice = async (req, res) => {
    ...
    const dir = path.join(process.cwd(), "/backend/latex/invoice");
